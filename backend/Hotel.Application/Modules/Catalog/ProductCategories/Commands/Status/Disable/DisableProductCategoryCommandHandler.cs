namespace Hotel.Application.Modules.Catalog.ProductCategories.Commands.Status.Disable;
    ...
    : IRequestHandler<DisableProductCategoryCommand, Unit>
{
    public async Task<Unit> Handle(DisableProductCategoryCommand request, CancellationToken ct)
    {
        ...
        if (cat is null)
        {
            throw new HotelNotFoundException($"Kategorija (ID={request.Id}) nije pronađena.");
        }
        ...
        {
            throw new HotelBusinessRuleException("category.disable.blocked.activeProducts",
