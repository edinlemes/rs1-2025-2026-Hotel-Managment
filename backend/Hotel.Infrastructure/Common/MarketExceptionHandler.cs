...
/// </summary>
public sealed class HotelExceptionHandler(
    ILogger<HotelExceptionHandler> logger,
...
) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        ctx.Response.StatusCode = ex switch
        {
            ...
            HotelConflictException or HotelBusinessRuleException => StatusCodes.Status409Conflict,
    ...
    private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
    {
        ...
        switch (ex)
        {
            case HotelNotFoundException:
            case HotelConflictException:
            case HotelBusinessRuleException:
                code = "entity.error";
                message = ex.Message;
                break;
